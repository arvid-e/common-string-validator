export function urlHasValidProtocol(url: string): boolean {
...
export function urlHasValidHostname(url: string): boolean {
  ...
  const hostnameString = hostnameMatch[0];

  if (hostnameString.length > 255) {
    return false;
  }
